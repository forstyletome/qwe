...
export default function _setHTMLSlides() {
  ...
  const IS_ACTIVE = constants.isActive;

  const { left } = this.container.getBoundingClientRect();
  const activeSlides = this.HTMLslides.filter(slide =>
    slide.classList.contains(IS_ACTIVE)
  );
  this.HTMLslides.forEach((slide, i) => {
    const slideWrap = slide;
    slideWrap.setAttribute(constants.index, i);
