...
class PixiSlider {
  constructor(props) {
    ...
    this._clearWrapper = _clearWrapper;
    this._addStage = _addStage;
    this._setHTMLSlides = _setHTMLSlides;
    this._loadPictures = _loadPictures;
    this._handleNav = _handleNav;
    this._createNav = _createNav;
  }
  ...
  _goTo(newIndex) {
    ...
    const tl = new TimelineLite({
      onComplete: () => {
        ...
        this.isPlaying = false;
        this.HTMLslides[newIndex].classList.add(constants.isActive);
        // remove slide text
    ...
    this.HTMLslides[this.active].classList.remove(constants.isActive);
    // add active slide title and remove previous
    this.canvas.stage.removeChild(this.canvas.stage.children[2]);
    this.canvas.stage.addChild(
      this.createTitle(this.slidesData.titles[this.active])
    );

    if (this.displacement !== undefined) tl.add(this.animate());
...
sliders.forEach(slider => {
  ...
  const mySlider = new PixiSlider(options);
  mySlider.init();
});
